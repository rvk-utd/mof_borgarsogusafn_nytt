 ...
 *   also be "maintenance_page" since borgarsogusafn_theme_preprocess_maintenance_page() calls
 *   this function to have consistent variables.
 */
function borgarsogusafn_theme_preprocess_html(&$variables, $hook) {
  // Add variables and paths needed for HTML5 and responsive support.
  $variables['base_path'] = base_path();
  $variables['theme_path'] = drupal_get_path('theme', 'borgarsogusafn_theme');
 ...
 */
function borgarsogusafn_theme_process_html(&$variables, $hook) {
 ...
 */
function borgarsogusafn_theme_preprocess_page(&$variables, $hook) {
  
  // add a variable for debugging,
  // like showing all region containers even if they have no content
  $variables['debug'] = FALSE;

  $path = drupal_get_path('theme', 'borgarsogusafn_theme') . '/assets/javascripts/';
  $options = array('scope'=>'footer');
  drupal_add_js($path . 'jRespond.min.js', $options);
  drupal_add_js($path . 'queries.js', $options);
  drupal_add_js($path . 'main.js', $options);
  drupal_add_js($path . 'jquery.bxslider.min.js', $options);
  drupal_add_js($path . 'slider.js', $options);
  drupal_add_js($path . 'jquery.matchHeight-min.js', $options);
  drupal_add_js($path . 'match-height.js', $options);
  drupal_add_js($path . 'item-list.js', $options);
  drupal_add_js($path . 'form.js', $options);
  drupal_add_js($path . 'faq-list.js', $options);
 ...
 *   An array of variables to pass to the theme template.
 */
function borgarsogusafn_theme_process_page(&$variables) {
 ...
 */
function borgarsogusafn_theme_preprocess_node(&$variables, $hook) {
 ...
 */
function borgarsogusafn_theme_preprocess_comment(&$variables, $hook) {
 ...
 */
function borgarsogusafn_theme_preprocess_taxonomy_term(&$variables) {
  // Add a suggestion based on view_mode.
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function borgarsogusafn_theme_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function borgarsogusafn_theme_process_block(&$variables, $hook) {
 ...
 *   An array of variables to pass to the theme template.
 */
function borgarsogusafn_theme_preprocess_panels_pane(&$variables) {
 ...
 */
function borgarsogusafn_theme_css_alter(&$css) {
...
*/
function borgarsogusafn_theme_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'scald_atom_add_form_add'){
    // This is the form displayed to a user at the "Add" stage of creating an atom
    // If we're creating an "Image upload" atom, Scald hard-codes
    // the form to display the scald_authors and scald_tags
    // Hide them as we don't use them
    if (array_key_exists('scald_authors', $form)) {
      $form['scald_authors']['#type'] = 'hidden' ;
    }
    if (array_key_exists('scald_tags', $form)) {
      $form['scald_tags']['#type'] = 'hidden' ;
    }
  }
}